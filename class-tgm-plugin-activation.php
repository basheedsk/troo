 ...
 * @version   2.6.1 for child theme Troo for publication on ThemeForest
...
<?php
			...
			if ($this->can_plugin_activate($slug)) {
				...
				if (is_wp_error($activate)) {
					...
					if (!$automatic) {
						...
						if (!isset($_POST['action'])) { // phpcs:ignore WordPress.Security.NonceVerification.DeprecatedWhitelistCommentFound
		...
		protected function is_core_update_page()
		{
			...
			if ('update-core' === $screen->base) {
				// Core update screen.
				return true;
			} elseif ('plugins' === $screen->base && !empty($_POST['action'])) { // phpcs:ignore WordPress.Security.NonceVerification.DeprecatedWhitelistCommentFound
				// Plugins bulk update screen.
				return true;
			} elseif ('update' === $screen->base && !empty($_POST['action'])) { // phpcs:ignore WordPress.Security.NonceVerification.DeprecatedWhitelistCommentFound
		...
		public function process_bulk_actions()
		{
			...
			if ('tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action()) {
				...
				if (isset($_POST['plugin'])) {
					if (is_array($_POST['plugin'])) {
						$plugins_to_install = array_map('sanitize_text_field', $_POST['plugin']);
			...
			if ('tgmpa-bulk-activate' === $this->current_action()) {
				...
				if (isset($_POST['plugin'])) {
					...
					$plugins = array_map('sanitize_text_field', $plugins); // Example sanitization function
				}
